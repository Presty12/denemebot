...
const kızrol = ayarlar.kızrol
const kızrol2 = ayarlar.kızrol2
...
üye.roles.add(kızrol)
üye.roles.add(kızrol2)
...
${emoji}Kullanıcıyı kaydeden yetkili ${message.author}, Verilen roller <@${kızrol}>, <@${kızrol2}>, Yeni ismi \`${neym} | ${eyç}\``)
.setColor("RANDOM")
...
.setDescription(`
${emoji} ${üye} adlı kullanıcının kaydı gerçekleşti. 
${emoji}Kullanıcıyı kaydeden yetkili ${message.author}, Verilen roller <@${kızrol}>, <@${kızrol2}>, Yeni ismi \`${neym} | ${eyç}\`, Kayıt kanalı ${message.channel.name}`)
.setColor("0x2f3136")
.setFooter(`Yetkilinin teyit bilgileri : 
Toplam \`${alldata}\` 
Toplam Kadın \`${allkızdata}\``)
log.send(logsave)
...
exports.conf = {
  ...
  exports.help = {
    name: 'kız',
    description: "Etiketlenen kişiyi kız rolleriyle kayıt eder.",
